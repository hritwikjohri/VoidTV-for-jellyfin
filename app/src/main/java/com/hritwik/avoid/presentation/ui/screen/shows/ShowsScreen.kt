package com.hritwik.avoid.presentation.ui.screen.shows
...
fun ShowsScreen(
    ...
    val continueWatching = remember(libraryState.resumeItems) {
        libraryState.resumeItems.filter { item ->
            item.type == ApiConstants.ITEM_TYPE_EPISODE || item.type == ApiConstants.ITEM_TYPE_SERIES
        }
    }
    ...
    when {
        ...
        !libraryState.isLoading && !hasContent -> {
            EmptyState(
                ...
                description = "New episodes and series will appear here once they're added to your library."
            )
        }
            ...
            LazyColumn(
                ...
                if (continueWatching.isNotEmpty()) {
                    item(key = "shows_continue_watching") {
                        BackdropRow(
                ...
                if (studios.isNotEmpty() && showLibrary != null) {
                    item(key = "shows_studios") {
                        StudioRow(
                            title = "Studios",
                            studios = studios,
                            serverUrl = serverUrl,
